    ...
    <section className="mx-auto max-w-[1100px] px-10 pb-[100px]">
      ...
      <div className="mt-14 grid grid-cols-2 gap-5">
        {features.map((f) => (
          <div
            key={f.title}
            className="rounded-[14px] border border-black/8 bg-[#f8f9fa] p-9 transition-all duration-300 hover:border-black/12 hover:shadow-lg"
          >
            <div className="text-[1.8rem]">{f.icon}</div>
            <h3 className="mt-[18px] font-display text-[1.1rem] font-bold text-[#111827]">
              {f.title}
            </h3>
            <p className="mt-[10px] text-[0.88rem] font-light leading-[1.65] text-[#6b7280]">
              {f.desc}
            </p>
          </div>
        ))}
